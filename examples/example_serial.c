 ...
 * @return <0: 失败
 */
int gm_time_get_current_timestamp_ms(uint64_t *timestamp_ms)
{
...
{
    gm_serial_t *gm_serial = gm_serial_create();
    if (gm_serial == NULL)
    {
    ...
    int ret = gm_serial_init(gm_serial, serial_name, E_BAUD_RATE_115200, 0, E_DATA_BIT_8, E_PARITY_BIT_N, E_STOP_BIT_1);
    if (ret != 0)
    {
    ...
    // 清空输入, 输出缓存
    ret = gm_serial_flush_both_cache(gm_serial);
    if (ret != 0)
    {
        printf("serial %s flush both data failed. ret: %d\n", serial_name, ret);
        gm_serial_destroy(gm_serial);
        gm_serial = NULL;
    ...
    // 串口发送数据
    ssize_t write_len = gm_serial_write_data(gm_serial, send_data, send_data_len);
    if (write_len < 0)
    {
    ...
    ssize_t read_len = gm_serial_read_data(gm_serial, recv_data, send_data_len, 1000);
    // 失败
    if (read_len < 0)
    {
        gm_time_get_current_timestamp_ms(&current_timestamp);
...
int main(int argc, char *argv[])
{
    ...
    if (argc > 1)
    {
        strncpy(serial_name, argv[1], sizeof(serial_name));
    }
    else
    {
        strncpy(serial_name, "/dev/ttyUSB0", sizeof(serial_name));
    }
